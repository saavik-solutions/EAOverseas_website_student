...
import { Edit3, Share2, Download, MapPin, GraduationCap, CheckCircle2 } from 'lucide-react';
...
export const ProfileHero: React.FC<Props> = ({ user }) => {
  return (
    <section className="relative w-full mb-12 pt-8">
       ...
       <div className="max-w-6xl mx-auto px-6 md:px-12 flex flex-col md:flex-row items-center md:items-center justify-between gap-8 bg-white border border-border rounded-[3rem] p-8 md:p-10 shadow-sm">
             ...
             <div className="space-y-4 w-full md:w-auto">
                <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-4 justify-center md:justify-start">
                   <h1 className="text-4xl md:text-5xl font-black text-text-primary tracking-tight">
                     {user.name} <span className="text-xl md:text-2xl font-bold text-text-muted">{user.nationality}</span>
                   </h1>
                   <Link 
                     href="/profile-intelligence"
                     className="inline-flex max-w-max mx-auto md:mx-0 px-4 py-1.5 bg-brand-primary/10 border border-brand-primary/20 rounded-full items-center gap-2 group hover:bg-brand-primary transition-all shadow-sm"
                   >
                      <div className="w-2 h-2 rounded-full bg-brand-primary group-hover:bg-white animate-pulse" />
                      <span className="text-[10px] font-black uppercase tracking-widest text-brand-primary group-hover:text-white transition-colors">PAI Score: {user.paiScore}%</span>
                   </Link>
                </div>
                ...
                <div className="flex flex-wrap items-center justify-center md:justify-start gap-x-6 gap-y-2 text-text-secondary">
                   <div className="flex items-center gap-2 text-sm font-bold bg-bg-base px-4 py-2 rounded-xl">
                      ...
                      {user.title} @ <span className="text-text-primary">{user.institution}</span>
                   </div>
                   <div className="flex items-center gap-2 text-sm font-bold bg-bg-base px-4 py-2 rounded-xl">
          ...
          <div className="flex flex-wrap items-center justify-center md:justify-end gap-3 w-full md:w-auto mt-4 md:mt-0">
             <button className="px-6 py-4 bg-brand-primary text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-brand-primary/90 transition-all shadow-md active:scale-95 flex items-center gap-2">
                <Edit3 className="h-4 w-4" />
                Edit Profile
             </button>
             <button className="p-4 bg-bg-base border border-border rounded-xl text-text-secondary hover:text-brand-primary hover:border-brand-primary/30 transition-all shadow-sm"><Share2 className="h-4 w-4" /></button>
