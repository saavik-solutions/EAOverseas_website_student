...
import { Edit3, Share2, Download, MapPin, GraduationCap, CheckCircle2, User as UserIcon, HelpCircle, Lock as LockIcon } from 'lucide-react';
             ...
             <div className="relative group shrink-0">
                <div className="w-32 h-32 md:w-36 md:h-36 rounded-full p-1.5 bg-bg-base shadow-sm border border-border/50">
                   <div className="w-full h-full rounded-full overflow-hidden relative bg-gray-50 flex items-center justify-center">
                      ...
                      {!user?.isLocked && (
                        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-all flex items-center justify-center cursor-pointer" onClick={onEdit}>
                           <Edit3 className="h-6 w-6 text-white opacity-0 group-hover:opacity-100 transition-all drop-shadow-md" />
                        </div>
                      )}
                   </div>
                </div>
                 {/* Verified & Locked Badges */}
                 <div className="absolute -bottom-1 -right-1 flex flex-col gap-1.5 items-end">
                    <div className="bg-brand-success text-white px-3 py-1 rounded-full shadow-lg border-2 border-white flex items-center gap-1">
                       <CheckCircle2 className="h-3.5 w-3.5" />
                       <span className="text-[9px] font-black uppercase tracking-widest">Verified</span>
                    </div>
                    {user?.isLocked && (
                      <div className="bg-slate-800 text-white px-3 py-1 rounded-full shadow-lg border-2 border-white flex items-center gap-1 animate-in slide-in-from-right-4">
             ...
             <div className="space-y-4 w-full md:w-auto">
                <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-4 justify-center md:justify-start">
                   ...
                   <div className="flex flex-wrap gap-2 justify-center md:justify-start">
                      <div className="inline-flex px-4 py-1.5 bg-brand-primary/10 border border-brand-primary/20 rounded-full items-center gap-2 group shadow-sm transition-all hover:bg-brand-primary">
                      ...
                      {user?.isLocked && (
                        <div className="inline-flex px-4 py-1.5 bg-slate-100 border border-slate-200 rounded-full items-center gap-2 text-slate-500">
                           <LockIcon className="h-3 w-3" />
          ...
          <div className="flex flex-wrap items-center justify-center md:justify-end gap-3 w-full md:w-auto mt-4 md:mt-0 relative z-10">
             {user?.isLocked ? (
                <div className="group relative">
                  <button 
                    disabled
                    className="px-6 py-4 bg-slate-100 text-slate-400 rounded-xl font-black text-[10px] uppercase tracking-widest flex items-center gap-2 cursor-not-allowed border border-slate-200"
                  >
                    <LockIcon className="h-4 w-4" />
                    Profile Locked
                  </button>
                  <div className="absolute bottom-full right-0 mb-4 px-3 py-2 bg-slate-800 text-white text-[9px] font-bold rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">
