...
export const CommunityFeed: React.FC = () => {
  ...
  const [activeCategory, setActiveCategory] = React.useState('All Discussions');
  ...
  const availableCategories = [
    { name: 'General Discussion', key: 'general' },
    { name: 'Visa & Immigration', key: 'visa' },
    { name: 'University Reviews', key: 'reviews' },
    { name: 'Financial Advice', key: 'finance' },
    { name: 'Work & Careers', key: 'careers' },
    { name: 'Social Events', key: 'social' },
  ];
    ...
    try {
      const res = await fetch('/api/feed', {
        ...
        body: JSON.stringify({
          ...
          flair: availableCategories.find(c => c.key === newCategory)?.name || 'Discussion'
        })
      });
      ...
      if (res.ok) {
        ...
        alert(errData.error || "Failed to publish post.");
      }
    } catch (error) {
      console.error(error);
  ...
  const filteredPosts = React.useMemo(() => {
    if (activeCategory === 'All Discussions') return posts;
    return posts.filter(p => p.category === activeCategory || p.flair === activeCategory || (activeCategory === 'visa' && (p.category === 'visa' || p.tags?.includes('visa'))));
          ...
          <div className="bg-white p-6 rounded-2xl border border-border shadow-sm mb-8 space-y-4 transition-all">
             <div className="flex gap-4 items-center">
               <input 
                  ...
                  className="flex-1 text-lg font-bold text-text-primary placeholder:text-text-muted/50 border-none outline-none focus:ring-0 px-0 bg-transparent"
               />
               <select 
                 value={newCategory}
                 onChange={(e) => setNewCategory(e.target.value)}
                 className="bg-bg-base text-[10px] font-black uppercase tracking-widest text-text-muted px-3 py-1.5 rounded-lg border border-border outline-none focus:border-brand-primary transition-colors cursor-pointer"
               >
        ...
        <div className="space-y-8">
          {filteredPosts.length === 0 ? (
            <div className="bg-white border text-center border-border rounded-2xl p-12">
              ...
              <h3 className="text-lg font-black text-text-primary mb-2">No discussions here yet</h3>
              <p className="text-sm font-medium text-text-muted max-w-sm mx-auto mb-6">
                Be the first to start a conversation in {activeCategory === 'All Discussions' ? 'the community' : activeCategory}! 
              </p>
              <button 
                onClick={() => {
                  if (activeCategory !== 'All Discussions') setNewCategory(activeCategory);
                  setIsComposing(true);
                }}
